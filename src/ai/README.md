...
`createAlgorithmicCandidateStrategy` is the credential-free candidate path for the MVP. It deterministically selects three prompts from canonical intelligence/profile/recent-title input, uses Minecraft-aware templates when Minecraft is selected or evidenced, otherwise stays game-neutral, labels them `algorithmic`, cites only fresh, high-confidence known canonical signal IDs when available, and never retains raw chat text or provider payloads.
