    ...
    return {
      ...
      viewer: viewerViewModelSchema.parse({
        ...
        privateRecovery: {
          ...
          viewerId: input.viewerId,
          acceptedCandidateId: input.acceptedCandidateId,
          acceptedAt: input.acceptedCandidateId === null ? null : input.envelope.occurredAt,
          sourceMode: input.acceptedCandidateId === null ? null : input.participationMode === "unavailable" ? null : input.participationMode,
          sessionPoints: input.sessionPoints,
          restoredAt: input.envelope.receivedAt,
        },
        hostedBoard: {
          status: "available",
          sessionId: input.session.sessionId,
          roomCode: "ABCDEFGH",
          url: "https://chatxpt.example/board/ABCDEFGH",
          qrImageUrl: null,
          expiresAt: null,
          message: "Use the hosted board only if the Twitch Extension is unavailable.",
        },
        chatVote: {
          status: input.capabilities.twitchChatVoting ? "not-delivered" : "unavailable",
          commandId: null,
          candidateId: null,
          receivedAt: null,
          message: input.capabilities.twitchChatVoting
            ? "Send 1, 2, or 3 in Twitch chat only when chat fallback is active."
            : "Twitch chat voting is unavailable for this session.",
          retryable: false,
        },
        questCycle: input.questCycle,
