...
export function buildMultiGameGameplaySnapshot(input: {
  ...
  const policyApproved = input.assessment.evidencePolicyStatus === "approved";
  const interpretationKnown = policyApproved && interpretation.status === "known";
  const groups: SignalCandidateGroup[] = [
    ...
    group(
      ...
      "activity-intensity",
      fallback,
      !interpretationKnown || input.assessment.motion === null
        ? null
        : {
  ...
  if (input.assessment.profile.gameId === "minecraft") {
    groups.push(
      group(
        ...
        policyApproved && input.assessment.minecraftHud?.status === "vanilla-like"
          ? {
              value: "vanilla-like",
  ...
  if (input.assessment.profile.gameId === "brawl-stars") {
    const confirmed = policyApproved && input.assessment.brawlHud?.status === "standard-like";
    groups.push(
      group(
        "brawl-hud-layout",
        "brawl-hud-layout",
  ...
  return buildGameplaySnapshot({
    ...
    capabilities: {
      tier: policyApproved ? input.assessment.supportTier : "universal-visual",
      gameId: input.assessment.profile.gameId,
      adapterId:
        policyApproved && input.assessment.supportTier === "calibrated-hud"
          ? input.assessment.profile.profileId
          : null,
      supportedSignals: policyApproved
        ? [...input.assessment.supportedSignals]
        : [...input.assessment.profile.universalSignals],
    },
    signals: groups,
    fusion: {
