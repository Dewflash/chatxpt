...
const fixtureEvidencePolicy = createMultiGameVisionEvidencePolicy({
  policyId: "fixture-multi-game-policy",
  calibrationEvidenceClass: "fixture",
  calibrationSourceIds: ["synthetic-quiet-action", "synthetic-transition-hud"],
  approvedProfileIds: [
    "generic-action-v1",
    "minecraft-java-vanilla-v1",
    "brawl-stars-standard-v1",
  ],
...
describe("multi-game evidence policy", () => {
  it("requires two calibration sources, a 0.75 floor, and at most three seconds of freshness", () => {
    expect(() => createMultiGameVisionEvidencePolicy({
      ...fixtureEvidencePolicy,
      calibrationSourceIds: ["only-one-source"],
    })).toThrow("at least 2 distinct");
    expect(() => createMultiGameVisionEvidencePolicy({
      ...fixtureEvidencePolicy,
      minimumConfidence: 0.74,
    })).toThrow("between 0.75 and 1");
    expect(() => createMultiGameVisionEvidencePolicy({
      ...fixtureEvidencePolicy,
      staleAfterMs: 3_001,
    })).toThrow("from 1 to 3000");
  });
});
...
describe("adaptive cadence and integrated multi-game analyzer", () => {
  ...
  it("advertises calibrated Minecraft support only when the fingerprint is recognized", () => {
    const recognizedAnalyzer = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy });
    ...
    const modded = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy }).analyse({
      frame: solidFrame(20),
      observedAt: 1_000,
      selection: selection("minecraft"),
    });
  ...
  it("requires HUD confirmations to agree inside the approved freshness window", () => {
    const analyzer = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy });
    analyzer.analyse({
      frame: vanillaLikeMinecraftFrame(),
      observedAt: 1_000,
    ...
    const staleAgreement = analyzer.analyse({
      frame: vanillaLikeMinecraftFrame(),
      observedAt: 4_001,
      selection: selection("minecraft"),
    });
    expect(staleAgreement).toMatchObject({
      supportTier: "universal-visual",
      minecraftHud: { status: "candidate-unconfirmed" },
    });
  });
  ...
  it("keeps universal motion meaning invariant across generic and calibrated profiles", () => {
    const genericAnalyzer = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy });
    const minecraftAnalyzer = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy });
    const base = texturedFrame();
...
describe("canonical multi-game snapshot projection", () => {
  it("keeps live-labelled facts unknown when only a fixture calibration policy is available", () => {
    const analyzer = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy });
    analyzer.analyse({
      frame: vanillaLikeMinecraftFrame(),
      observedAt: 1_000,
    ...
    const fixtureFrame = observation(0, 1_200);
    const liveFrame: GameplayFrameObservation = {
      ...fixtureFrame,
      envelope: {
        ...fixtureFrame.envelope,
        source: "obs-virtual-camera",
        evidenceClass: "live",
      },
    };
    const snapshot = buildMultiGameGameplaySnapshot({ frame: liveFrame, assessment });

    expect(assessment).toMatchObject({
      evidencePolicyStatus: "evidence-class-mismatch",
      evidencePolicyId: null,
      supportTier: "universal-visual",
    });
    expect(snapshot.envelope.evidenceClass).toBe("live");
    expect(snapshot.capabilities).toMatchObject({ tier: "universal-visual", adapterId: null });
    expect(snapshot.signals.every(({ observation }) => observation.status !== "known")).toBe(true);
  });
  ...
  it("refuses to combine a frame with analysis from another timestamp", () => {
    const assessment = new MultiGameVisionAnalyzer({ evidencePolicy: fixtureEvidencePolicy }).analyse({
      frame: texturedFrame(),
      observedAt: 1_000,
      selection: selection(null),
    });
