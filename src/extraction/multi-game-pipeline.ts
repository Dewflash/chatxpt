...
import type { MultiGameVisionEvidencePolicy } from "./vision-evidence-policy";
...
export function createMultiGameGameplayExtractionPipeline(
  ...
  const analyzer = new MultiGameVisionAnalyzer({ evidencePolicy: options.evidencePolicy });
  return {
    async *snapshots(source: FrameSource, signal?: AbortSignal) {
      for await (const output of streamMultiGameVisionAssessments(source, {
        ...
        if (output.status === "ready") {
          ...
          yield unavailableSnapshot(
            output.frame,
            analyzer.resolve(selected(options.selection, output.frame)),
          );
        }
      }
    },
