...
The Twitch viewer keeps local selection separate from authoritative acknowledgement. A pending command locks every option, an accepted private receipt locks and highlights the accepted card, and influential tallies appear only after that receipt or after the cycle becomes active/terminal. The compact Extension layout keeps its confirmation and engagement area reachable while quest cards scroll. Active and terminal states prioritise the authoritative winning quest, progress, result, cooldown, community hype, and private session points without calculating any of them locally. Typed `DomainError` props pause uncertain commands, preserve the selected quest, and expose one optional retry or Twitch reauthentication callback supplied by Role 1.

The hosted board uses the same viewer shell, while the chat fallback and OBS overlay render the same authoritative open-vote, active, terminal, cooldown, and reconnect phases in formats appropriate to those surfaces. The overlay remains read-only and can show all three authoritative choices and tallies during voting plus the sanitized public `upNext` line when Core provides one. Countdown expiry produces only a neutral waiting message; tie, zero-vote, winner, reward, and lifecycle resolution remain server-owned.
