...
`DefaultCandidateAssembler` is the Phase 3 pre-engine safety boundary. Role 1 should call it after `DefaultInterventionPolicy` permits a cycle and before issuing `system.intelligence-ready`. It requires a selected game, accepts zero or more untrusted candidate values, rejects objectives that do not explicitly identify that game, replaces rejected or missing values from a seeded game-aware fallback library, re-validates diversity and history, and returns either exactly three canonical options with an audit trail or a typed failure. Minecraft fallbacks use known Minecraft mechanics without asserting unsupported current state. It never semantically repairs a rejected objective or weakens safety rules.
...
`DefaultDirectorCueConverter` is the pure LD-R3-03 bridge from that consumed `converted` cue into the existing `DefaultCandidateAssembler` and `DefaultQuestEngine` proposal path. Missing, malformed, provider, algorithmic, and deterministic candidates all face the same validator; rejected or absent candidates receive separately validated seeded fallbacks. Success stops at a canonical private `proposed` cycle with exactly three options and explicit approve/reject actions. Any invalid cue/context, fallback exhaustion, or proposal-boundary failure returns typed `no-publication`; it never starts voting or publishes directly to viewers.

`DefaultLiveDirectorProposalCoordinator` is the R3-014 public composition adapter injected into Role 1's sole orchestrator. It invokes the configured Role 2 candidate port when current canonical gameplay is available, treats provider failure as normal null input for deterministic fallback, and returns only a Role 3 proposal decision or typed no-publication error. The orchestrator—not this adapter—atomically persists the converted cue and proposed cycle, stamps one revision, and broadcasts role-safe projections.
...
`PROVIDER_QUALITY_RUBRIC.md` is Role 3's evaluation gate for the D-072-approved OpenAI `gpt-5.6-terra` path. `provider-quality.ts` makes the batch-level hard gates, weighted threshold, and critical minimums deterministic. Neither file calls or integrates a provider. Role 2 must still supply real operational trials for the exact model, one-attempt eight-second limit, privacy/credit boundary, cancellation, and fallback behaviour; until then provider quality remains unproven and the credential-free path stays viable.

`outcomes.ts` is the pure Phase 5 progress and terminal-reward policy. Manual progress cannot regress. Automatic partial progress requires an explicit deterministic allowed-signal rule on the authoritative active quest, an exactly matching context copy, and known, supported gameplay evidence that is no older than 15 seconds and at least 0.75 confidence; a missing, manual, or mismatched active rule fails closed, and audience sentiment never proves gameplay completion. The policy rejects conflicting or cross-game evidence and refuses progress during a known scene transition, menu, cutscene, or inactive match. Broad universal visual signals such as motion or scene state may inform partial progress but cannot independently prove completion.

R3-015 adds the D-060 terminal predicate without weakening those gates. A signal rule may carry an explicit signal kind, `equals`/`at-least`/`at-most` comparison, target, calibrated game ID, and distinct corroborating signal kinds. Role 3 validates the proposed rule against the selected game and current supported capabilities, copies the winning rule into active authority, and accepts automatic value `1` only when the system command supplies an exactly matching rule and current gameplay evidence proves the predicate and every required corroborator. Candidates or legacy states without a predicate remain parseable but manual-only. The engine never derives a predicate from objective text, an allowed signal kind, or a system-requested value, and it never infers automatic failure. Manual succeed/fail/cancel/skip therefore remains the safe fallback whenever evidence is insufficient. Cancellation/skip/expiry award neither points nor hype, while a failed attempt awards zero points and +2 hype. Role 1 still owns authentication, idempotency, persistence, realtime projection, and command replay; its orchestrator persists Role 3's terminal reward and shared-hype delta atomically so a duplicate command cannot award twice.
