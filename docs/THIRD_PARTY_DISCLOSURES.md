...
| `openai` | `package.json` / `package-lock.json` | D-072-approved server-side Responses adapter for structured quest candidates | Server only; no `NEXT_PUBLIC` provider key allowed | Exact model `gpt-5.6-terra`; source/configuration is not proof of a live call, and missing credit/credential preserves algorithmic fallback. |
...
| OpenAI API | D-072 permits opt-in server-side candidate generation with exact model `gpt-5.6-terra` | Team-owned `OPENAI_API_KEY` stays server-side. The model has no API free tier, so enable it only with existing prepaid/promotional credit; no contributor must purchase quota and new spend requires a separate owner decision. | Send only bounded normalised gameplay/audience/profile/history context; exclude raw frames/chat, usernames, Twitch IDs, viewer identity, and secrets. ChatXPT sets `store: false`, retains no prompt/output/vendor payload, and discloses OpenAI's default abuse-monitoring retention of up to 30 days unless the project has Zero Data Retention. Timeout, malformed output, refusal, rate limit, outage, or missing credit/credential falls back algorithmically before Role 3 validation. |
