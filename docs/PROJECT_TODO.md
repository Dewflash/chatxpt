...
| P-006 | P0 | BLOCKED | Role 4 | Working Streamer Studio and compact Twitch control surface use accepted contracts. | Role 2 build plan and Role 4 feasibility review; P-003 | First-time setup, persistent preferences, live controls, health/errors, responsive evidence. |
| P-007 | P0 | BLOCKED | Role 5 | Working Twitch viewer, hosted fallback, chat fallback, and OBS visual surfaces use accepted contracts. | Role 2 build plan and Role 5 feasibility review; P-003 | Multi-device voting, active quest/progress/results, fallback evidence, responsive evidence. |
...
| P-009 | P0 | BLOCKED | Role 1 | Golden Twitch workflow passes end to end using real captured gameplay and real Twitch activity. | P-004 through P-008 | Stream setup -> OBS Virtual Camera frames -> derived signals -> three quests -> streamer behaviour -> vote -> overlay -> result/reward, with confidence, unknown, algorithmic/AI, and fallback labels; no simulated live evidence. |
