...
**Next pass:** execute `role-2/real-input-evidence` against Role 1's merged browser `FrameSource`: two authorised gameplay samples with separate annotations, quiet/action/transition measurements, latency/resource observations, sanitised real-chat evidence, and honest unknown behaviour. D2-07 through D2-11 are approved; selective OCR remains `unknown` until PR #75 is safely rebased and merged, and diagnostic calibration cannot become live evidence without a separate live-labelled run.
...
- Fixture tests cover valid, partial, overfull, invalid, duplicate, incorrectly labelled, timeout, refusal, rate-limited, unavailable, generic-error, cancellation, invalid-fallback, and metrics cases. The credential-free algorithmic strategy is merged; real provider/model trials, Role 3 scoring, and the joint D23 recommendation remain open.
...
- Focused tests currently use diagnostic-shaped fixture data only. Two user-authorised Brawl Stars clips are source-inspected with separate relative-time annotations and local-only hashes; raw clips, sampled frames, and visible player handles remain outside Git. Role 1's browser `FrameSource` is merged, but a browser-delivered live execution, the safely merged Tesseract.js dependency from PR #75, sanitised real Twitch chat, resource observations, privacy review, artifact, and evidence-manifest entry remain required.
