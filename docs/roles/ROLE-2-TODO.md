...
| R2-010 | P0 | IN PROGRESS | Upgrade the Minecraft-aware game-state schema, pixel-aware HUD/activity detection, and AI context contract under D-086 and `docs/build-plans/MINECRAFT-AWARE-IMPLEMENTATION-PLAN.md`. | D-086; R2-002/R2-003; Role 1 gameplay ingress | Fixture implementation now covers schema, dynamic/search-based Minecraft-like HUD observation, two-matching-frame promotion for health/hunger/armor/hotbar facts, conservative selected-hotbar-category detection, visual menu/sleep detection, repeated lower-health damage confirmation, low-health danger, conservative sleep/recovery/exploration activity, selected-item mining/building hints, broad hostile-like and environment hints without exact mob/damage/menu claims, reusable generic `gameState` mapping, Minecraft-specific `gameFacts`, active ChatXPT quest transport, fresh/confidence-qualified provider citations, Minecraft-only algorithmic/deterministic fallback, and Role 3 validation on the automatic proposal path. Real vanilla/modded Minecraft evidence, selected item category accuracy beyond coarse visual classes, menu/sleep OCR or exact crafting distinction, exact mining/building/combat classification, exact damage cause, exact hostile mob type, and exact biome/environment claims remain evidence-gated follow-up before live accuracy claims. |
