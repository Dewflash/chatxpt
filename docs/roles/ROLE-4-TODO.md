...
| R4-003 | P0 | IN PROGRESS | Build full Studio module and self-service Twitch connection/setup flow for Role 1's thin routes. | R4-001; UI-X01/UI-X02/UI-X05; issue #140; P-017/ICP-01 | `/studio` now mounts the ICP-01 product Home shell, with dedicated authenticated routes for Gameplay Engine, Live Analytics, Live Quests, Profile & Defaults, Stream Settings, and Test Lab. Product pages use shared availability copy so unfinished controls stay inert with a streamer-facing reason and next step. Home now renders distinct cannot-connect, ready/no-stream, preparing, live, reconnecting, and ended compositions, and its Start/End controls dispatch only when the existing session service capability allows them. The existing full management/control surface remains available to compact Twitch/Studio companion surfaces while ICP pages grow into the accepted product flow, and its visible copy has been scrubbed of revision/role/contract/diagnostic-route wording. D-065 supplies a secure manual broadcaster-session start and real channel mapping; full OAuth/EventSub automation remains open. |
...
| R4-008 | P0 | IN PROGRESS | Produce contract, responsive, accessibility, and failure-state evidence. | R4-003 through R4-006 | Issue #140 evidence is joined by R1-025 private Live Director tests for known/stale/permission/loading/offline/reconnect states and exact 420/1440 px fixture renders with zero horizontal offenders (`E-20260819-R1-003`). Fixture tests now verify the persistent stream-context surface is read-only and omits command controls; ICP-01 product-shell tests verify customer-facing navigation and unavailable states. The 20 August copy/availability cleanup updated assertions but was not run by Codex at owner request. Real Twitch/OBS dock, accessibility-device, and final setup-to-live evidence remain. |
