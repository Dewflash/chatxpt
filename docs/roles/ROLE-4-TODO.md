...
| R4-003 | P0 | IN PROGRESS | Build full Studio module and self-service Twitch connection/setup flow for Role 1's thin routes. | R4-001; UI-X01/UI-X02/UI-X05; issue #140; P-017/ICP-01 | `/studio` now mounts the ICP-01 product Home shell, with dedicated authenticated routes for Gameplay Engine, Live Analytics, Live Quests, Profile & Defaults, Stream Settings, and Test Lab. Product pages use shared availability copy so unfinished controls stay inert with a streamer-facing reason and next step. The existing full management/control surface remains available to compact Twitch/Studio companion surfaces while ICP pages grow into the accepted product flow, and its visible copy has been scrubbed of revision/role/contract/diagnostic-route wording. D-065 supplies a secure manual broadcaster-session start and real channel mapping; full OAuth/EventSub automation remains open. |
