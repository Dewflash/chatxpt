...
| R4-005 | P0 | IN PROGRESS | Build pre-stream testing and integration-health experience. | UI-X01/UI-X05/UI-X09; issue #140 | Test Lab exposes live capture plus key-free permanent setup for the broadcaster-linked OBS Browser Source and private Live Director. Studio now makes the Desktop Live Director the primary private setup path while retaining the same URL as an OBS Custom Dock fallback. The local macOS package launches as a small always-on-top window with encrypted linking, saved bounds/opacity, click-through, global hide/show, all-Spaces display, and optional login launch; focused and full checks pass. Canonical `/studio/gameplay` combines OBS Virtual Camera/direct screen connection, exact watched-feed preview, detector proof, and stats in one same-tab page; `/studio/gameplay/capture` redirects there. Permission-denied/device-loss plus real owner linking, Minecraft focus, and capture evidence remain owner-run. |
