...
| R4-002 | P0 | READY | Publish the shared visual-system public entry point, then expand accessible base components. | R4-001 | Minimum tokens/components reach Role 5 early through a stable entry point; later expansion does not require Role 5 to edit/copy Role 4 files. |
...
| R4-006 | P0 | BLOCKED | Build compact Twitch Config and Live Config modules. | UI-X02/UI-X05/UI-X06; Role 1 D1-08 resolved by D-056 | Focused embedded UI supports required stream-time commands without duplicating Studio or inventing permissions/lifecycle logic. |
