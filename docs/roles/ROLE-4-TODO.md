...
evidence remains owner-run, and the owner has deferred the dedicated post-stream
analytics/history product.
...
| R4-003 | P0 | IN PROGRESS | Build full Studio module and self-service Twitch connection/setup flow for Role 1's thin routes. | R4-001; UI-X01/UI-X02/UI-X05; issue #140; P-017/ICP-01 | `/studio` now mounts the unified product Home plus Gameplay Engine, Live Analytics, Live Quests, Profile & Defaults, Stream Settings, and Test Lab. State-specific Home and recovery copy use authoritative capabilities. Twitch connection now uses state-bound OAuth, channel-game import, and EventSub subscription creation; the signed Studio cookie preserves verified authority. The manual form remains collapsed for diagnostic credential recovery. Final external Twitch and visual evidence remains open. |
| R4-004 | P0 | IN PROGRESS | Build persistent streamer profile and challenge customisation. | UI-X02/UI-X03; issue #140 | Profile authority now persists game, safety/restriction/accessibility/watchlist fields, four editable starter presets plus named custom presets, voting/reward presentation, selected preset, and current-stream overrides. Effective preset/override values reach candidate context. Focused tests pass; reconnect and real-provider evidence remain in the final external workflow. |
| R4-005 | P0 | IN PROGRESS | Build pre-stream testing and integration-health experience. | UI-X01/UI-X05/UI-X09; issue #140 | Test Lab exposes live capture and key-free OBS Browser Source setup. `/studio/gameplay/capture` refreshes short-lived grants, remembers the selected profile, retries automatically when camera permission is already granted, and remains open as a persistent capture tab while Studio pages change. Permission-denied/device-loss and real OBS evidence remain owner-run. |
