...
| R1-006 | P0 | BLOCKED | Register Twitch app and Extension test version. | R1-011 | OAuth callback placeholder, server-safe setup readiness API, and exact Viewer/Config/Live Config shells are implemented without enabling token exchange. Remaining: real Twitch developer-console registration evidence, test channel/allowlist, Local or Hosted Test evidence, Extension JWT validation, EventSub/chat wiring, and recorded live runtime proof. |
