...
**Next:** finish R1-026 source verification and continue the remaining viewer/OBS payoff work. R3-009 through R3-013 are merged, and `codex/minecraft-schema-decisions` now source-wires cue conversion plus accepted-live-gameplay eligible-cycle proposal requests through the shared server runtime. Final tests, real Twitch/OBS/Supabase evidence, and owner acceptance remain open.
...
| R1-007 | P0 | IN PROGRESS | Run the early OBS capture spike, then define/test capture and Browser Source integration. | R1-003 provisional port | Studio now opens session-bound Gameplay Capture at `/studio/gameplay/capture` and generates a secure read-only `/obs-overlay` descriptor. Virtual Camera snapshots cross the authenticated ingress with current authority; the capture page refreshes short-lived grants while running and remembers browser-local capture profile/last-success metadata without persisting frames. Setup sessions remain `preparing` until explicit Start sees current Twitch/Game Capture readiness and commits the lifecycle transition to `live`. Overlay tokens stay in the URL fragment and project canonical read-only state. Focused source tests pass on prior capture branches, and current readiness-gated expectations are updated but not executed per owner instruction. Outstanding evidence still requires macOS permission, real frame sampling, full browser/device reconnect recovery, and a recorded OBS Browser Source run. |
...
| R1-021 | P0 | IN PROGRESS | Mount authenticated normalised gameplay snapshots into the shared server runtime without per-frame revision churn. | R1-003/R1-004/R1-007; Role 2 extraction public output | A server-only setup key issues short-lived session grants; memory/Supabase keep one monotonic snapshot per active session; ingestion rejects stale or cross-session/cycle/revision/evidence input; `/studio/gameplay/capture` submits only normalized facts and refreshes authority while running; and the sole orchestrator hydrates matching gameplay with explicit Capture Health. Focused checks pass on prior capture branches. Full repository checks and real browser/OBS evidence remain before handoff. |
...
| R1-026 | P0 | IN PROGRESS | `LD-R1-04`: preserve Extension Vote/Active/Result and compressed OBS payoff, with minimal Catch-up isolated as a P1 experiment. | R1-023; Role 3 lifecycle/conversion passes | Source wiring now keeps personal receipt/recovery private, preserves hosted/chat fallback paths, routes `turn-into-vote` and accepted-live-gameplay eligible-cycle proposals through the shared runtime before streamer approval, and does not add OBS private cue/context fields. Final responsive/accessibility/reconnect/terminal fixtures, viewer/OBS payoff proof, and real evidence remain owner-run before `DONE`. |
