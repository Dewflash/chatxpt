...
**Next:** finish review of the R1-015 UI gateway PR and land R1-016 viewer recovery/fallback seams, then activate the team-owned Supabase Free preview. Continue coordinating open feasibility handoffs and keep ambiguous legacy moves deferred; AI/extraction, quest mechanics, and detailed UI implementation remain with their owners.
...
| R1-016 | P0 | IN PROGRESS | Implement private per-viewer recovery plus hosted-board discovery and Twitch-chat delivery seams. | R1-003/R1-004; UI-X07/UI-X08/UI-X10; D1-06D/D1-06E | Contract schemas, memory fallback, and server-side Supabase adapters now cover private viewer recovery, sanitised shared broadcasts, hosted-board room-code discovery, and explicit chat-vote acknowledgement statuses on `codex/role-1-viewer-recovery-seams`; live Supabase cloud and real Twitch chat adapter evidence remain separate. |
