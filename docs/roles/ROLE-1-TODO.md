...
| R1-006 | P0 | BLOCKED | Register Twitch app and Extension test version. | R1-011 | Source now implements self-service OAuth, validated broadcaster/channel-game import, EventSub subscription creation, signed chat delivery, ordinary-chat aggregation, and exact `1`/`2`/`3` fallback votes. Remaining external work is developer-console registration, deployed HTTPS callback/allowlist configuration, Local or Hosted Test, and recorded live proof. |
| R1-007 | P0 | IN PROGRESS | Run the early OBS capture spike, then define/test capture and Browser Source integration. | R1-003 provisional port | Studio now opens session-bound Gameplay Capture at `/studio/gameplay/capture` and generates a secure read-only `/obs-overlay` descriptor. Virtual Camera snapshots cross authenticated ingress with current authority; capture refreshes short-lived grants and remembers only the current session's browser-local profile/last-success metadata without persisting frames. Setup remains `preparing` until Start sees current Twitch/Game Capture readiness. Overlay tokens stay in the URL fragment. The full 95-file/753-test repository gate, production build, key-free URL browser check, stale-session recovery, and desktop/mobile layout checks pass. Outstanding evidence still requires macOS permission, real frame sampling, device reconnect recovery, and a recorded OBS Browser Source run. |
