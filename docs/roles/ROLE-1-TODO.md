...
| R1-022 | P0 | IN PROGRESS | Validate and scope the proposed Live Director expansion, then activate only the retained passes in its draft implementation plan. | P-015; D-074; current product and tool access | Secondary research substantiates or rejects each named pain; official/current capabilities and alternatives are audited; streamer/Extension/OBS additions are traced to quantitative and qualitative evidence; the private in-game cue channel receives a go/no-go; and the owner accepts a keep/defer/reject scope. `docs/build-plans/LIVE-DIRECTOR-IMPLEMENTATION-PLAN.md` exists but no proposed source pass becomes active before that gate. |
