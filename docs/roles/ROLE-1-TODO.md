...
| R1-007 | P0 | IN PROGRESS | Run the early OBS capture spike, then define/test capture and Browser Source integration. | R1-003 provisional port | Browser-side OBS Virtual Camera `FrameSource` adapter, exact-device safety, read-only Browser Source descriptor, and a Role 2-composed diagnostic route are fixture/browser-tested. OBS Virtual Camera started locally, but macOS denied the selected browser camera access; outstanding evidence still requires that permission, real OBS frame sampling, and Browser Source overlay verification. |
...
| R1-021 | P0 | IN PROGRESS | Mount authenticated normalised gameplay ingress into the shared server runtime. | R1-007; Role 2 extraction public output; PRs #138/#139 | A server-only bootstrap key issues short-lived session grants; bounded live/diagnostic OBS snapshots match current authority, enter memory/Supabase latest-state storage, and are available to the sole orchestrator without raw-frame persistence. Real browser/OBS evidence remains a later owner-led run. |
| R1-021 | P0 | IN PROGRESS | Mount current gameplay snapshots into the authoritative runtime without per-frame revision churn. | R1-003/R1-004/R1-007; `codex/multigame-extraction` | Memory/Supabase keep one monotonic snapshot per active session; ingestion rejects session/revision/cycle/evidence mismatch; frame stamping follows realtime revision/cycle changes; the orchestrator hydrates matching gameplay at command time; capture health is explicit; focused and full checks pass. Authenticated browser-to-server ingress and real OBS evidence remain separate exits. |
