...
| R1-022 | P0 | IN PROGRESS | Validate and scope the proposed Live Director expansion, then activate only the retained passes in its draft implementation plan. | P-015; D-074; `docs/research/LIVE-DIRECTOR-SECONDARY-RESEARCH.md`; project-owner scope acceptance | Secondary research now substantiates the narrow attention/orchestration problem, audits current capabilities and substitutes, traces additions to quantitative and qualitative gates, and recommends the private cue channel plus keep/defer/reject scope. The remaining acceptance condition is the owner's scope ruling; no proposed source pass is active. |
