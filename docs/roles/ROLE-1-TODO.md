...
| R1-007 | P0 | IN PROGRESS | Run the early OBS capture spike, then define/test capture and Browser Source integration. | R1-003 provisional port | Browser adapter, explicit OBS-only diagnostic selection, ephemeral frame provenance/release, permission/unavailable/stale/ended handling, tests, and setup guidance are implemented on `role-1/obs-capture-spine`. OBS 32.2.1 is installed, but macOS Camera Extension approval is required before a real frame can be sampled; no hardware success is claimed. Browser Source output remains a later part of R1-007. |
