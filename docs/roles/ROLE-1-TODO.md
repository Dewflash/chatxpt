...
**Next:** run owner-operated real OBS Dock/Browser Source and Twitch-issued Extension evidence for R1-026. The source pass now gives Live Director a permanent private broadcaster URL, keeps the OBS overlay on its permanent broadcaster URL, publishes one privacy-safe chat/game/explainer context to all three surfaces, and preserves voting, winner reveal, activation, result, and cooldown across automated runtime checks. Real Twitch/OBS/Supabase evidence and owner acceptance remain open.
...
- Check local setup/start/reset/reconnect first: `npm run dev:twitch` wakes the app; Test Lab reset disconnects Twitch/session/capture, clears the local fake account and obsolete bypass, and returns this browser to the mandatory prefilled ChatXPT sign-in; Connect Twitch resumes only after that sign-in through the intended Twitch/OAuth path.
...
| R1-007 | P0 | IN PROGRESS | Run the early OBS capture spike, then define/test capture and Browser Source integration. | R1-003 provisional port | Canonical `/studio/gameplay` now combines OBS Virtual Camera/direct screen connection, exact watched-feed preview, live detector proof, and stats in one same-tab Studio page; legacy `/studio/gameplay/capture` redirects there. The exact visible preview element is now the frame sampler for both capture choices, the selected source name/type stays visible, and capture cannot claim connected until playback plus a real-sized frame succeed. One persistent Studio shell now keeps that same capture/analyser instance mounted during client-side navigation across Home, Analytics, Quests, Profile, Stream Settings, and Test Lab; explicit Stop, reset/sign-out, source loss, or leaving/reloading Studio still ends capture. On 22 August 2026, real OBS Virtual Camera output carrying team-owned Minecraft crossed authenticated gameplay ingress into an authoritative Twitch-mapped Studio session and produced exactly three deterministic fallback quests while unsupported calibrated facts stayed unknown. Authenticated local OBS WebSocket inspection confirmed the signed overlay as an enabled 1920×1080 Browser Source, captured its clean transparent idle render, and later captured a completed canonical quest from an explicitly diagnostic fake-viewer cycle. Capture refreshes short-lived grants, remembers only session-local browser metadata, and does not persist frames. Overlay tokens stay in the URL fragment; the local overlay setup key was rotated after rehearsal. Direct-screen operator evidence after this preview hardening, device reconnect recovery, and a real Twitch-issued voting/active/result overlay run remain open. |
...
| R1-021 | P0 | IN PROGRESS | Mount authenticated normalised gameplay snapshots into the shared server runtime without per-frame revision churn. | R1-003/R1-004/R1-007; Role 2 extraction public output | Studio authorises short-lived session capture grants without exposing signing keys; memory/Supabase keep one monotonic snapshot per active session; ingestion rejects stale or cross-session/cycle/revision/evidence input; canonical `/studio/gameplay` submits only normalised facts and refreshes authority while running; the persistent Studio shell preserves the active media/analyser/delivery instance across internal page navigation; and the sole orchestrator hydrates matching gameplay with explicit Capture Health. Automated repository checks pass; real browser/OBS navigation evidence remains before final handoff. |
...
| R1-026 | P0 | IN PROGRESS | `LD-R1-04`: preserve Extension Vote/Active/Result and compressed OBS payoff, with minimal Catch-up isolated as a P1 experiment. | R1-023; Role 3 lifecycle/conversion passes; D-095 | Source wiring now keeps personal receipt/recovery private; gives the 420px Live Director Dock and 1920x1080 OBS Browser Source permanent broadcaster-linked grants that resolve the active session; projects the same privacy-safe chat health/energy, gameplay status, explainer, hype, and quest state to Live Director, OBS, Extension, hosted board, and chat fallback; persists 30/60-second Automatic viewer voting and directly publishes Manual streamer selections as active without creating a vote or voting countdown; and holds winner/result states for 10 seconds. Studio Home no longer imitates these public surfaces: its two public tabs mount the canonical Role 5 Twitch Extension and OBS components from the current authoritative streamer revision, while the already-visible private Live Director is omitted. Personal viewer identity, receipt, points, and open-vote tallies remain absent from the Studio preview. Real OBS Dock/Browser Source, Twitch-issued Extension, responsive/accessibility-device, and Supabase evidence remain owner-run before `DONE`. |
...
| R1-029 | P0 | IN PROGRESS | Execute persistent Twitch-backed streamer profile/preset ownership and established local fallback. | D-087/D-089/D-091/D-092; existing project verification; `docs/build-plans/STREAMER-PROFILE-PERSISTENCE-PLAN.md` | Safe profile bootstrap/load, account/identity backfill, A/B isolation, browser/restart persistence, local established account/presets, conflict-safe cloud recovery, authorization, secrets, and golden-flow regression evidence pass. |
| R1-030 | P1 | READY | Execute one-entry-per-stream history, retained analytics, authorized APIs, and Studio history delivery. | R1-029 ownership exit; D-087/D-088/D-090; `docs/build-plans/STREAM-HISTORY-ANALYTICS-PLAN.md` | Stream-ID/lifecycle deduplication, terminal summary, legacy limitations, retention, A/B authorization, UI, Supabase Cloud, Vercel Preview, second-browser/restart, and golden-flow regression evidence pass. |
| R1-031 | P0 | IN PROGRESS | Deliver the private always-on-top Desktop Live Director companion. | R1-025/R1-026; D-093/D-094/D-095/D-096; existing permanent broadcaster grant | Studio links the installed companion once; the macOS app securely stores the broadcaster grant, resolves later sessions, stays above windowed gameplay, supports click-through/hide/show and saved bounds, and retains browser/OBS Dock fallbacks. Profile & Defaults persists automatic/manual capture-time setup; successful capture sends only `chatxpt://open`, so the app reuses its encrypted local grant without exposing it to the capture page. The compact 2x2 status grid includes live duration, truthful chat state, and a header gameplay state. Its narrow command scope can generate proposals; Automatic shows no candidate selector and pushes all three to viewers, while Manual selects one current candidate and starts it without viewer voting. An active quest exposes only confirmed `cancel` and immediate `succeed` when Role 3 supplies those actions; the server re-resolves the broadcaster/session/cycle/revision, rejects other lifecycle commands, persists the terminal result, and broadcasts the shared revision. An owner capture run proved the initial source smoke had incorrectly assigned `chatxpt` to generic `com.github.electron`; registration is now packaged-only, source runs repair that development association, and native LaunchServices plus direct-open evidence targets `com.chatxpt.live-director`. Owner-linked real Twitch commands in both modes, both active outcomes, and one post-rebuild capture-time launch remain required before `DONE`. |

## Current R1-029 explicit local fallback evidence

- Local development now accepts `CHATXPT_PERSISTENCE_MODE=memory` to bypass a configured but unavailable or schema-incompatible Supabase project without deleting its credentials.
- The same server-side memory runtime owns the verified Twitch session, gameplay ingress, quest state, viewer fallbacks, and OBS resolution for that local process; Twitch OAuth remains the real broadcaster authority.
- Supabase realtime is disabled in this mode and Studio reports degraded local persistence instead of claiming cloud durability. Browser-local established-account presets remain available through the existing explicit local profile flow.
- Focused environment, realtime-access, and Studio session tests pass. Real owner-operated Twitch OAuth and OBS capture remain the hands-on acceptance step.
...
- The continued rehearsal proved current authorised Twitch chat and real OBS Minecraft can jointly reach candidate generation despite gameplay advancing revisions faster than the audience window. Same-cycle audience signals retain their original freshness/confidence while being composed at the proposal revision; local-algorithmic Minecraft proposals now survive validation. Publication also compares the streamer's public quest projection with viewer/overlay state, so private rationale no longer produces a false `realtime delivery is recovering` result.
- The same live run exposed a 10-second memory-mode Hosted Board polling gap across the fresh cycle identity: server recovery was correct, but an already-open client could miss most of the next vote unless manually refreshed. The recovery poll is now 1.5 seconds, matching the short reveal/transition cadence while retaining server-authoritative reads.
- The continued 23 August rehearsal completed a real-input credential-free cycle from current Twitch chat and OBS/Minecraft through exactly three proposals, hosted anonymous voting, winner activation, success, 100-point reward, community-hype update, cooldown, and matching OBS overlay API revisions. Twitch-issued Extension JWT voting was not exercised.
- The same run exposed two cross-role presentation/capture defects. Numeric gameplay intensity is now translated to bounded `quiet`/`transition`/`active` viewer copy instead of leaking a raw float. OBS Virtual Camera can still freeze on an old frame while the browser sampler reports advancing frame counts; a stop/reconnect recovered the feed, but automatic frozen-feed detection remains open under R1-007.
...
- Viewer rewards are session-scoped and private, shared hype remains shared,
  ...
  Post-stream analytics/history is a separate required initiative under D-087
  and is not claimed by this demo-critical pass.
