...
| R1-021 | P0 | IN PROGRESS | Mount authenticated normalised gameplay snapshots into the shared server runtime without per-frame revision churn. | R1-003/R1-004/R1-007; Role 2 extraction public output | A server-only setup key issues short-lived session grants; memory/Supabase keep one monotonic snapshot per active session; ingestion rejects stale or cross-session/cycle/revision/evidence input; and the sole orchestrator hydrates matching gameplay with explicit Capture Health. Focused checks pass on the finals integration branch. Full repository checks and Role 2 review remain before handoff; real browser/OBS evidence stays under R1-007. |
