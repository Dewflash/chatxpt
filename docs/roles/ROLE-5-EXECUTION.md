...
| Current TODO | R5-002 through R5-007A (`IN PROGRESS`) |
...
| Role 1 override: design-system consumption and fallback-state hardening | `codex/role-5-demo-surfaces` / PR #95 | Public viewer, hosted-board, chat fallback, and overlay surfaces now consume Role 4's public design-system root/components/tokens; the board takes a Role 1 dispatcher and voter key instead of embedding the fixture dispatcher in the production component. Stable wrapper exports now match the accepted Role 5 mount names: `TwitchViewerPanel`, `HostedQuestBoard`, `TwitchChatVoteInstructions`, and `QuestOverlay`. Hosted board access states and chat acknowledgement statuses now render from Role 1 supplied presentation data without adding room/chat authority to Role 5. OBS overlay fixtures and rendering now distinguish inactive, voting, active, result, and reconnecting states instead of presenting voting as an active winner. Public viewer copy now avoids Role 1/Role 5 terminology unless the diagnostic fixture label is explicitly enabled. The board now displays server-supplied community hype/session XP and emits `viewer.react` only when an authorised reaction dispatcher is provided. | `npm run test -- src/viewer/surfaces-render.test.ts src/viewer/surface-model.test.ts tests/integration/role-entrypoints.test.ts`; earlier PR #95 pass also ran `npm run test -- src/viewer/surface-model.test.ts src/viewer/surfaces-render.test.ts src/design-system/design-system.test.ts tests/integration/role-entrypoints.test.ts`; `npm run typecheck`; `git diff --check`; viewer CSS/copy scan for old hardcoded palette/fixture wording; `npm run check` | Fixture-only UI and local source/test/build evidence; no real Twitch Extension auth, Supabase realtime, multi-viewer, authorised reaction dispatcher, or OBS Browser Source evidence. | Push PR #95 update and request drdexe review; next pass should replace fixture snapshots with the accepted Role 1 runtime path. |
