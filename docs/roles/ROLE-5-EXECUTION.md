...
| Current TODO | R5-002 (`IN PROGRESS`; Role 4 design-system handoff is available on main, first render modules are in review) |
| Source editing allowed now | Fixture-rendered viewer, hosted-board, chat-fallback, and read-only overlay modules may consume `@/design-system`; route mounting and realtime dispatch remain Role 1 work |
...
| R5-P02 design-system render slice | `codex/role-5-viewer-next-slice` / pending | Added public Role 5 viewer, hosted-board, chat-fallback, and OBS overlay render modules consuming Role 4's design-system entry point from main. Inactive overlay stays visually quiet; degraded viewer snapshots disable commands; chat fallback maps exactly three visible options without parsing chat. | Fixture render tests for viewer surfaces; pending full `npm run check` | Fixture-only server-render evidence; no real Twitch Extension, hosted-board access, realtime dispatch, Supabase, or OBS Browser Source claim | Role 1 route/harness PRs and Role 5 review are still required before visible integrated evidence |
