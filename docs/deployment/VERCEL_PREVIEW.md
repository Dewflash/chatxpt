...
7. Run `npm run check:client-secrets` against the built output in the same environment or a downloaded deployment artifact. The scanner reports variable names only and must not print secret values.
8. Record the deployment URL, command, source revision, reviewer, and limitations in `docs/evidence/manifest.json` before citing it as deployment evidence.
