...
| Authenticated gameplay ingress | `Dewflash`; open contribution under D-071 | Current `main`; former stacked [PR #139](https://github.com/Dewflash/chatxpt/pull/139) and [PR #141](https://github.com/Dewflash/chatxpt/pull/141) closed after their heads were confirmed contained in `main` | Exchange a server-only setup key for a short-lived session capture grant, ingest only bounded current OBS-derived normalised snapshots, and expose them to the sole orchestrator through shared memory/Supabase persistence | In progress; source is integrated, while real browser/OBS/Supabase evidence remains open |
