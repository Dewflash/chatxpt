...
| Browser-safe UI gateway and harness | `Dewflash` | `role-1/ui-gateway-harness`; UI-X01 #17, UI-X02 #18, UI-X05 #21 | Typed setup/readiness and command results, protected six-surface fixture host, shared component/browser verification, and no Role 4/5 product UI | Needs review |
| Hosted-board and Twitch-chat fallback seams | `Dewflash` | `role-1/hosted-chat-seams`; issues #23/#24 | Safe room-code/share exchange, short-lived hosted-viewer grants, neutral fallback-message contracts, and Twitch-confirmed delivery receipts without UI-owned authority | In progress |
