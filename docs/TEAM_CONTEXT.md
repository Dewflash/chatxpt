...
| Role 2 provider/fallback evaluation | `joelyrk` | `role-2/provider-fallback-evaluation`; Role 3 rubric on `main` | Build provider-neutral timeout/failure metrics and injected algorithmic recovery, then obtain Role 3 scoring before a joint recommendation to Role 1 | In progress |
