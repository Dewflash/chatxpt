...
| Persistent streamer profile and local fallback | `Dewflash`; Role 4 UI contribution | `codex/profile-implementation-check`; R1-029; D-087/D-089/D-091/D-092 | Load verified Twitch profiles before session bootstrap, preserve revisions/presets across sessions, add future-compatible account ownership, retain the established device-local profile fallback, and expose truthful account/input/persistence health without changing the golden flow | In progress; source, forward migrations, memory/Supabase adapters, Studio health, local conflict recovery, and focused regression tests are implemented. Real Supabase Cloud, Vercel Preview, second-browser, restart, and owner-visible evidence remain open. |
...
| Twitch/OBS golden rehearsal recovery | `Dewflash`; cross-role validator and viewer-recovery fixes disclosed to Roles 3/5 through this shared record | Current `main`; evidence `E-20260822-R1-002`; 23 August local follow-up | Prove real Twitch lifecycle/chat, authoritative OBS Virtual Camera gameplay, exactly-three fallback generation, participation, and truthful shutdown while repairing revision races found under load | In progress; real Twitch online/offline and aggregate chat, authoritative Minecraft capture, three fallback quests, one hosted vote through winner activation, an installed/active Twitch Local Test panel, and a clean idle OBS Browser Source render are proven. The follow-up fixed false Minecraft pause/HUD classification and a live-discovered quest-cycle identity reuse that blocked the same hosted viewer in the next cycle; focused two-cycle recovery now passes. An explicitly diagnostic two-viewer run also proved duplicate protection, progress, success/reward, unsupported-evidence rejection, and a completed canonical OBS render. A current-run Twitch chat aggregate, real Twitch-issued winner/JWT vote, unpaused HUD reading, active/result OBS composite, and durable cloud persistence remain open; Vercel/Supabase work is paused by owner direction. |
