...
| Role 2 provider/fallback evaluation | `joelyrk`; open contribution | `role-2/openai-structured-output-fix`; [PR #61](https://github.com/Dewflash/chatxpt/pull/61); issue #132; D-072 | Evaluate approved OpenAI `gpt-5.6-terra` with strict exactly-three output, 8-second timeout, privacy/credit disclosure, citation freshness/confidence, and algorithmic plus Role 3 deterministic fallback | In progress; an owner-authorised generic Brawl diagnostic returned exactly three provider candidates in 5,091 ms, but Role 3 accepted only two. The partial run is recorded without prompt/output retention; production-quality, OBS/Minecraft, and hard-gate evidence remain open. |
| AI input/output contract hardening | `joelyrk`; Role 1/3 shared-seam notification | `role-2/openai-structured-output-fix`; previous input/output fixes merged through PR #167 | Carry the current known streamer goal and active quest through candidate generation for every game, reject duplicate citations, classify OpenAI refusal output correctly, and keep the strict provider schema within the supported subset | In progress; the provider schema no longer emits unsupported `uniqueItems`, runtime duplicate rejection remains authoritative, and the prompt now prevents rationales from naming unsupported fact categories as disclaimers. Automated checks pass; the next provider trial must confirm all three candidates pass Role 3. |
...
| OBS calibrated sampling repair | `Dewflash`; Role 2 extraction responsibility | `codex/fix-obs-calibrated-sampling` | Keep generic sampling bounded while allowing the calibrated Minecraft product path to retain enough pixel structure for its 640×360 HUD analysis | Needs review; rebased current `main`, a real OBS Virtual Camera run analysed 138 frames with 136 accepted snapshots while unsupported HUD facts remained `unknown`, development and production smoke pass, and the full repository gate passes 107 test files / 845 tests plus production build. Integration is pending. |
