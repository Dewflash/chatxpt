...
| Viewer experience | `drdexe`; open contribution | `role-5/real-evidence-run`; source commit `79192dc`; integrates `role-5/finals-viewer-review` | Preserve canonical Extension/hosted/chat/overlay UX while proving two-viewer authoritative vote-to-winner/OBS parity first, then reconnect and fallbacks under settled D5-16 | In progress; `E-20260819-R5-001` adds a passing production-server memory rehearsal, while real Twitch-issued identity/EventSub, Supabase two-device recovery, actual interaction, and OBS Browser Source remain owner-resource-dependent R5-008 evidence |
| Live Director implementation | `Dewflash` and `L0pch` (five passes each) | `role-1/live-director-02-live-context`; source commit `90726e6`; [PR #152](https://github.com/Dewflash/chatxpt/pull/152); [issue #150](https://github.com/Dewflash/chatxpt/issues/150); `docs/roles/ROLE-1-RESUME-HANDOFF.md`; P-016; R1-023..R1-027; R3-009..R3-013; D-075/D-076 | Implement the accepted falsifiable audience-to-sidequest loop: Role 1 owns authority/delivery/integration/evidence and Role 3 owns deterministic cue/quest mechanics, while preserving source truth, exactly-three validation, private/public projection boundaries, and every participation fallback | In progress; LD-R1-01 and LD-R1-02 are complete with fixture-only evidence `E-20260819-R1-001`/`002`; the canonical Role 3 input is `DeclaredStreamIntent` plus `AudiencePointer`, including stale and permission-denied states, and open PR #151 must import that seam rather than retain temporary private shapes; R1-025 waits for/deconflicts R3-010 cue actions; direct ChatXPT value remains unproven until comparative real-input evidence |
