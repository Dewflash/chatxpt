...
| Quest lifecycle runtime ticks | `Dewflash`; Role 1 scheduling with Role 3 engine authority | `codex/fix-quest-lifecycle-ticks`; D-044 | Advance expired active quests and terminal/cooldown states through the canonical system tick without UI-owned lifecycle logic | Needs integration; a real Twitch/OBS/Minecraft cycle exposed a terminal result stuck beyond the 120-second cooldown. The shared runtime repair is wired into Studio, Twitch Extension, hosted-board, and OBS reads; focused tests pass 5 files / 24 tests and the full gate passes 108 files / 847 tests plus production build. |
