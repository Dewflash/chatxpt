...
| App state single source of truth | `Dewflash`; Roles 2-5 affected | `docs/build-plans/APP_STATE_SINGLE_SOURCE_OF_TRUTH.md` | Final integration-hardening pass so Studio, Gameplay Engine, Live Director, viewer surfaces, fallbacks, and OBS consume the same canonical health/freshness/fallback/revision story | Planned after feature branches land; do not add new local health calculations when a shared projected state exists |
