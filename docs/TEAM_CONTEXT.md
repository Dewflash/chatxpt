...
| Real OBS browser capture spine | `Dewflash` | `role-1/obs-capture-spine` | Explicit OBS Virtual Camera selection, ephemeral real-frame delivery, truthful capture states, recursion guard, and Role 2 `FrameSource` handoff | In progress; real hardware/browser run required |
...
| Role 2 provider/fallback evaluation | `joelyrk` | `role-2/provider-fallback-evaluation`; [PR #61](https://github.com/Dewflash/chatxpt/pull/61); Role 3 rubric on `main` | Build provider-neutral timeout/failure metrics and injected algorithmic recovery, then obtain Role 3 scoring before a joint recommendation to Role 1 | In progress |
...
| Quest engine implementation | `L0pch` | `role-3/quest-ai-policy`; PR pending | Engine boundary, lifecycle, safety, fallback, and provider-neutral quest AI objectives/validation policy | In progress |
