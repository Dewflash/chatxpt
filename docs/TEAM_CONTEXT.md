...
| Role 2 visual extraction spike | `joelyrk` | `role-2/real-fixture-spike`; Role 1 dependency `role-1/obs-capture-spine` | Consume the canonical ephemeral `FrameSource` through downsampled visual measurements and selective-region OCR plumbing; record fixture-only evidence until authorised gameplay and a browser-delivered OBS frame are available | Needs review |
