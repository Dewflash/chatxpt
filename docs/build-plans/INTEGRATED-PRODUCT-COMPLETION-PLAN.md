...
| Production Game Capture | `/studio/gameplay` combines capture connection, OBS Virtual Camera/direct screen selection, a named exact-feed preview, live detector proof, and Studio-authorised ingress without exposing setup keys or opening a second Studio tab. The visible preview is also the frame-sampling element, and the page does not claim connected before playback plus a real-sized frame succeed. The legacy `/studio/gameplay/capture` route redirects to this canonical page. Real operator proof for both capture choices after this hardening remains open. | ICP-02 |
...
| Automatic Live Director refresh | Accepted gameplay ingress now asks the shared runtime to emit a canonical Live Director context refresh when the same snapshot did not already submit a quest proposal; accepted ordinary-chat snapshots also publish canonical audience signals. Final real-input evidence remains open. | ICP-04 |
