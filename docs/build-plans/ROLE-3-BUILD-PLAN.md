...
| D3-06 | Proposed/approval/veto/automatic activation behaviour | Accepted; superseded by D-095 | Quest routing follows the effective profile mode. `Automatic` lets Studio or the private Desktop Live Director generate exactly three validated options and push the whole batch to viewer voting without choosing a candidate; the authoritative winner activates automatically. `Manual` requires the streamer to select one current validated option and starts it directly without viewer voting, a voting countdown, or tallies. Reject cancels the whole batch. |
...
| D3-11 | Fallback taxonomy, selection, seeding, and history sensitivity | Accepted; quest-copy boundary corrected by D-080 | Use a curated game-aware library spanning low-risk strategy, commentary, teaching, reflection, and focus patterns, organised by selected game profile and supported mechanics. Order it by stable hash of Role 1-supplied session/cycle seed, validate against the same profile/evidence/history rules, use no ambient randomness, never present an unknown or contradicted state as current, and return typed exhaustion instead of relaxing game compatibility, safety, feasibility, or repetition. |
...
| ID | Owner decision | Status | Recorded answer |
| --- | --- | --- | --- |
| D3-20 | Quest objectives/instructions Role 2's model context must optimise for | Accepted; quest-copy boundary corrected by D-080 | Request exactly three meaningfully distinct, game-aware quests that are understandable at a glance, compatible with the selected game, feasible in the current evidenced moment, measurable within the current match, and calibrated to the streamer's profile, audience state, duration, difficulty, and reward settings. Each candidate must include a concise title, one unambiguous instruction, a duration or completion condition, difficulty, session points, and a producer-only rationale grounded only in known supported signals. A candidate may use a mechanic known for the selected game but may not claim an unknown state is current. Never optimise spectacle over safety, consent, accessibility, teammate welfare, or streamer restrictions, and never invent or contradict gameplay facts. |
...
**R3-015 implementation record (2026-08-21):** The canonical completion rule now supports an optional explicit predicate containing a signal kind, equality or ordered comparison, target, game identity, and distinct corroborating signal kinds. Role 3 rejects proposed predicates that do not match the selected game and current supported capabilities, copies only the winning validated rule into active state, and permits a system-requested terminal success only when the active and supplied validation contexts match exactly and fresh known evidence proves the predicate plus corroboration. Existing confidence, freshness, conflict, game, scene-transition, menu, cutscene, inactive-match, and broad-visual ambiguity gates remain stronger than completion. Candidates and legacy active states without predicates remain valid but manual-only; the engine does not infer predicates from objective text or provider intent. The application orchestrator atomically persists Role 3's terminal reward and shared-hype delta and preserves duplicate-command no-double-award behavior across reconnect projections. Verification is fixture-labelled and algorithmic only; no real gameplay, Twitch, OBS, OpenAI, or hosted Supabase run is claimed.
### Phase 7 acceptance
