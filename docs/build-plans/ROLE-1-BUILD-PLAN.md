...
| D1-06C | Browser-safe UI command/read client, local role-surface harness, and shared UI verification stack | Resolved by D-054 | Use versioned JSON Route Handlers and an injected typed fetch client; HTTP-only same-origin sessions or scoped bearer capabilities; an explicitly enabled, fixture-labelled, production-disabled six-surface harness; Vitest/jsdom/Testing Library for component interaction and Playwright/Chromium for browser and screenshot evidence. |
| D1-06D | Private per-viewer vote acknowledgement, session-points read model, and reconnect recovery without shared-state leakage | Resolved by D-055; implementation remains R1-P06B | Return an immediate typed command result and restore personal state through an authorised snapshot fetch; use verified Twitch identity or an opaque session-scoped HTTP-only anonymous grant; keep shared realtime sanitised and omit a private realtime channel from the MVP. |
| D1-06E | Hosted-fallback discovery/share path and Twitch-chat announcement/acknowledgement policy | Resolved by D-056; implementation remains R1-P06B | Exchange the eight-character room code or safe share URL for an authorised session grant without durable URL tokens; optional QR encodes the same URL; chat open/result templates are authoritative while individual counted/rejected/late acknowledgements are best-effort, one per viewer/cycle, rate-limited, and claimed only after Twitch delivery succeeds. |
