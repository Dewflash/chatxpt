...
| D1-06C | Browser-safe UI command/read client, local role-surface harness, and shared UI verification stack | Resolved by D-060 | Use Role 1's `FetchUiGatewayClient` pattern with same-origin/no-store/typed command results/current revisions and no UI business-rule authority. Keep the diagnostic harness fixture-only, visibly labelled, production-gated, and mounted across Studio, Twitch Config, Twitch Live Config, Viewer Board, hosted-board, and OBS overlay surfaces with schema, command, jsdom, browser screenshot, production-gating, and full-check verification. |
| D1-06D | Private per-viewer vote acknowledgement, session-points read model, and reconnect recovery without shared-state leakage | Resolved by D-061 | Private accepted choice/source/points/reconnect expiry live only in authorised per-viewer receipts for matching authenticated or anonymous-token viewers; shared snapshots/history stay sanitised and aggregate-only; duplicate votes preserve the first accepted choice without double-counting. |
| D1-06E | Hosted-fallback discovery/share path and Twitch-chat announcement/acknowledgement policy | Resolved by D-062 | Hosted fallback returns a bounded session-scoped direct board link, share data, and optional QR without requiring QR/account creation. Role 1 owns chat parsing/delivery/rate limits/templates; Role 5 presents supplied status only; chat acknowledgement status is claimed only after delivered outbound Twitch acknowledgement. |
