...
7. External model-provider adoption is no longer open for the judged MVP. D-055 adopts no external provider, uses the credential-free algorithmic candidate route plus Role 3 deterministic validation/replacement, and leaves Groq only as a future controlled trial candidate.
