...
- **Summary:** Added typed browser read/command contracts and client, setup readiness, six explicitly fixture-only local hosts, reproducible auth/failure scenarios, and shared component/browser verification. Successful snapshots now enforce active actor/surface invariants, and rejected token retrieval returns a typed dependency failure.
...
- **Verification:** `npm run test:ui` (21 focused tests); `npm run test:e2e` (eight Chromium flows and screenshot); `npm run check` (14 files/94 tests plus production build); `npm audit` (zero vulnerabilities); `git diff --check`; production start with the diagnostic flag deliberately enabled still returned 404 for both page and API.
