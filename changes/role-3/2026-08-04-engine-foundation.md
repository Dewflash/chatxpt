...
- **Summary:** Adds the pure quest lifecycle foundation, Phase 2 intervention/control policy, Phase 3 safety-first exactly-three assembly, and a bounded Phase 4 voting window while deferring winner activation behind the shared close-vote seam.
...
- **Verification:** `npm.cmd exec vitest run src/quest-engine` passes 37 focused tests. `npm.cmd run check` passes lint, typecheck, role boundaries, all 110 tests, and the production build.
