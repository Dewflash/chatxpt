...
- **Role:** Role 3
- **Issue/PR:** #50 / #143
- **Summary:** Automatic partial quest progress now requires a matching signal rule on the authoritative active quest and fails safely on missing/manual/mismatched rules, ambiguous visuals, contradictory or stale evidence, cross-game snapshots, and transition/menu/cutscene/inactive-match contexts; under D-060 a system progress request with value 1 is rejected until a persisted predicate-bearing rule exists, so only manual success can award rewards or write terminal history.
